...
if(sessionStorage.visita != '' && sessionStorage.visita != null && sessionStorage.visita != undefined){
    ModalHome('fecha');
    $('.btn-whats-fixo').css('display','block');
}else {
    ModalHome('abre');
...
// FORMULARIO CONTATO DE LPS
jQuery(".contatoLP").submit(function(){

    var dados = new FormData(this);

    var url = Dados.url + "ajax/contato_lp";

    $(".btnEnviaForm").disabled = true;
    $(".btnEnviaForm").innerHTML = "Enviando, aguarde...";
    ...
    $.ajax({
        ...
        {
            $(".btnEnviaForm").disabled = false;
            $(".btnEnviaForm").innerHTML = "Agendar contato";

            if(data.tipo == true)
            {
                // Limpa os Campos dos formulários
                $('.contatoLP').each (function(){this.reset();});


                setTimeout(function () {
                    location.href = Dados.url + "lead-sucesso";
                },1500)
   ...
   $.ajax({
      ...
      success: function(data)
      {
        ...
        data.forEach(function(x){
         ...
         // item2 += "<a href='https://api.whatsapp.com/send?l=pt_BR&phone=5508005809771&text=Olá! Vi no site que vocês atendem " + x.nomecategoriaservico +" e gostaria de agendar um horário' target='_blank'><button style='margin: 10px 0px 20px 0px;'>Agendar via WhatsApp</button></a>";
         ...
         item2 = "<div class=\"botoes\">" +
                    ...
                    // "<a href='https://api.whatsapp.com/send?l=pt_BR&phone=5508005809771&text=Olá! Vi no site que vocês atendem " + x.nomecategoriaservico +" e gostaria de agendar um horário' target='_blank'><button class=\"btn-whats\">Agendar pelo WhatsApp</button></a>" +
                 "</div>";
         ...
         if(Dados.loginUsuario == true)
         {
           if (x.valorservicoreferencia != null && x.valorservicoreferencia  != '')
           {
              ...
              {
                item3 = "<span class=\"preco\">"+ x.valorservicoreferencia  +"</span>";
              }
           }
           else
           {
         ...
         if(id == 146)
         {
           ...
           // item2 += "<a href='https://api.whatsapp.com/send?l=pt_BR&phone=5508005809771' target='_blank'><button style='margin: 10px 0px 20px 0px;'>Agendar por WhatsApp</button></a>";
           // item3 = "<span class=\"preco\"></span>";
...
function ModalAgendaExame(acao,titulo)
{
  ...
  else
  {
    ...
    var html = "<a href='https://api.whatsapp.com/send?l=pt_BR&phone=5508005809771&text=Olá! Vi no site que vocês atendem " + titulo +" e gostaria de agendar um horário' target='_blank'>" +
...
function ModalValor(acao,id)
{
    var aux = null;

    if(acao == "fecha")
    {
        $("#modalValor" + id).css("opacity", "0");

        setTimeout(function(){
            $("#modalValor" + id).css("display", "none");
        },500);
    }
    else
    {

        $("#modalValor" + id).css("display", "flex")

        setTimeout(function(){
            $("#modalValor" + id).css("opacity", "1");
        },500);
    }
}

$(".selectMais").on("change", function(){
  ...
  $("#tituloServico" + id).css("display", "block");
  $("#valor" + id).html("" + valor);
